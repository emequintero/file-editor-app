...
app.get("/files", (request, response)=>{
    ...
    availableFilesAsBinary.forEach(file=> {
        ...
        //add the file to the availableFiles array
...
app.post("/files", (request, response)=>{
    ...
    //managing files using fileID makes it so we can have several files with the same name 
    //without overwriting them
    let fileID = Date.now().toString();
    //add the fileID to the payload
    ...
    //create a JSON file in the available-files folder with the file payload data using fs
    fs.writeFileSync("available-files/" + fileID + ".json", fileFriendlyData);
...
app.put("/files/:fileID", (request, response)=>{
    let fileIDWeAreLookingFor = request.params.fileID;
    //path for the file we're looking for (Example: available-files/12345.json)
    let pathForFile = "available-files/" + fileIDWeAreLookingFor + ".json";
    //stringify the payload so we can write it to a JSON file
